...
def main():
    ...
    for r,cat in list(found.values())[:cfg['limits'].get('max_repositories',250)]:
        ...
        c.execute('INSERT OR REPLACE INTO repositories VALUES(?,?,?,?,?,?,?,?,?,?,?)',(r['id'],r['full_name'],r['name'],r['html_url'],r.get('description'),r.get('language'),lic,json.dumps(r.get('topics',[])),cat,discovered,now))
        a=score(r,cat,growth); total,g,ls,mkt,saas,ai,loc,best,fast,long=a
